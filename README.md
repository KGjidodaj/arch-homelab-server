...
* **Storage Management & Log Rotation:** To ensure long-term hardware longevity of the Chromebook's eMMC storage and prevent disk exhaustion. Global Docker log rotation is enforced via /etc/docker/daemon.json. Containers are restricted to a maximum log size with limited rolling backups. Preventing runaway log files from consuming host storage.
...
* **Docker Daemon Isolation (Socket Proxy):** To mitigate the risks of container breakout, the raw Docker socket (/var/run/docker.sock) is never exposed to web-facing diagnostic tools. Instead, a restricted proxy acts as an API firewall. Tools like Dozzle connect to this proxy via an internal TCP network, which is explicitly configured to allow strictly read-only access (GET requests for logs). While at the same time aggressively blocking any POST commands or access to container environment variables/secrets.
