...
* **Nginx Proxy Manager (NPM):** Serves as the reverse proxy, handling SSL/TLS termination, enforcing strict Access Control Lists (ACLs) and HTTP Strict Transport Security (HSTS). It routes traffic internally without unnecessarily exposing container ports to the host. Even its own Admin panel (port 81) is completely removed from public exposure, routed exclusively through a secured domain with strict ACLs. All hosted domains have successfully achieved an A+ security rating on SSL Labs testing ensuring enterprise-grade encryption.
